...
int main(){
    ...
    {
        BaseFile myFile("test_BaseFile.bin", "wb");     
        ...
        assert(written==13 && "written error");
        assert(myFile.tell()==13 && "tell error");
    }

    {
        BaseFile myFile("test_BaseFile.bin", "rb");
        assert(myFile.is_open() && "is_open error");
        assert(!myFile.can_write() && "can_write error");
        assert(myFile.can_read() && "can_read error");
    ...
    cout << "BaseFile tests PASSED" << endl;  
    return 0;
}