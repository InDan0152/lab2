...
int main() {
    ...
    // все будет проинициализированно нулями, как в пустом конструкторе (s=nullptr, len =0)
    // может быть проиницивализированно все, согласно моим конструкторам (MyString() | MyString(const MyString& my) | MyString(const char* s) )
    /**
     ...
     * 
     * Вызов функции write_int для производых классов (Base32File, RleFile) даст тот же самый результат,
     * что и код, написанный вручную, если в базовом классе BaseFile метод write объявлен как
     *  виртуальный (virtual).
     */
    {
    BaseFile bf("1.txt", "wb");
    ...
    write_int(bf, val);    // Запишет в обычный файл
    write_int(b32f, val);  // Запишет в Base32
    write_int(rf, val);    // Запишет с RLE-сжатием
    }
    /**
     ...
     * RleFile? Почему?
     * это означает что мы говорим компилятору "подожди. сразу не решай какой код вызвать. как дойдешь до нужной строчки 
     * посмотри на реальный тип объекта в памяти и вызови его версию функции"
     * (технически) в классе создается таблица виртуальных методов (vtable), а в каждом объекте — скрытый указатель на эту таблицу (vptr).
     * ----
     * размер увеличился, потому что компилятор автоматически добавил скрытое поле (vptr).
     * этот указатель нужен, чтобы объект "знал" где находятся таблица с его методами
     * -----
     * Как изменилось поведение при вызове функции `write_int` для производных
     * классов? Почему?
     *  Функция стала «умной». теперь, когда она вызывает file.write(), она обращается к vptr объекта.
     * -----
     * начал работать деномический полиморфизм. теперь поведение функции зависит не от типа ссылки, а от типа 
     * реального объекта на который она указывает 
     */
     ...
     */
        ...
        {
        BaseFile *files[] = { 
            new BaseFile("2_7__bf.txt", "wb"), 
            new RleFile("2_7_rf.txt", "wb"), 
            new Base32File("2_7_b32f.txt", "wb"), 
        };
     ...
     * логику, используя массив указателей на объекты базового класса.
     -при инициализации массива объектов создаются временные копии, чьи деструкторы закрывают файлы раньше времени.
     ---------
     -использование массива объектов базового типа для хранения производных типов нарушает арифметику указателей 
     (sizeof(BaseFile) != sizeof(Base32File)), из-за чего итерация по массиву обращается к мусору в памяти.

     *

     {
        BaseFile *base_files = new BaseFile[2] { BaseFile("2_8__bf1.txt", "wb"), BaseFile("2_8__bf2.txt", "wb") };
        BaseFile *b32_files = new Base32File[2] { Base32File("2_8_b32f1.txt", "wb"), Base32File("2_8_b32f2.txt", "wb") };
        for (int i = 0; i < 2; ++i) {
    ...
    files[0] = new BaseFile("2_8_bf1.txt", "wb");
    files[1] = new BaseFile("2_8_bf2.txt", "wb");
    files[2] = new Base32File("2_8_b32f1.txt", "wb");
    files[3] = new Base32File("2_8_b32f2.txt", "wb");
     ...
     * --мы не можем создавать объекты этого класса
     * 
    ...
    {
        Base32File2 b32f(new BaseFile("b32f.txt", "wb"));
        RleFile2 rf(new Base32File("rle_b32f.txt", "wb"));
        write_int(b32f, 1234567);
        write_int(rf, 1234567);
    } 
    ...
    {
    IFile* choeto = new Base32File2(new Base32File2(new RleFile2(new BaseFile("result.txt", "wb"))));

    const char* message = "Hello World!";
    choeto->write(message, 12);
